...
- **Places365 场景识别**：365 种场景，`img_scene()` / `img_scene_top()` / `img_is_indoor()`。
- **人物聚类**：人脸嵌入 + DBSCAN，侧栏按人分组、可重命名。
- **相簿与智能相簿**：手动归组与可重算的 DSL 动态集合。
...
- [功能列表](/tio/features) — 检测、场景、直方图、EXIF、标签、聚类、相簿
- [DSL 语法参考](/tio/dsl-reference) — 数据类型、运算符、集合、扩展
- [内置宏参考](/tio/macros) — 60+ 内置宏完整清单
- [系统架构](/tio/architecture) — 组件、数据流、缓存算法
- [模型系统](/tio/models) — 基座 / 扩展 / 场景包与缓存格式
- [桌面 GUI](/tio/gui) · [CLI 与 REPL](/tio/cli) · [模型导出](/tio/model-export)
- [常见问题](/tio/faq) — 缓存、模型、场景、聚类排查
- [下载 Tio](/download) — 最新 Release 附件
